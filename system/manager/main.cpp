...
void initLog(FILE *out, GameState &gameState) {
  ...
  fprintf(out, "### SamurAI 3x3 Game Log ###\n"
               ...
               "\"SamurAI 3x3\" %d %d %d %d\n",
          setting.turns, gameState.battleField.width,
          gameState.battleField.height, setting.cureTurns);
  ...
  for (int s = 0; s != 6; s++) {
    ...
    fprintf(out, "\n");
    fprintf(out, "%d %d %d %d\n", role.vision, role.activity, role.homeX,
            role.homeY);
  }
}
...
[[noreturn]] static void invOpt(char *argv[], int k,
...
[[noreturn]] static void helpMessage(char *argv[]) {
...
static string stringOption(int argc, char *argv[], int &argIndex) {
  ...
  while (*(++arg) != '\"') {
    ...
    if (*arg == '\0') {
      invOpt(argv, 0,
             "Invalid option argument for option " + string(argv[argIndex]));
    }
    ret += *arg;
  }
  return ret;
...
static void processOptions(int argc, char *argv[]) {
  ...
  if (ax != 6)
    invOpt(argv, 0, "Six -a options should be specified");
  if (px != 6)
    invOpt(argv, 0, "Six -p options should be specified");
  if (ux != 6)
    invOpt(argv, 0, "Six -u options should be specified");
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    for (turn = 0; turn != gameState.setting.turns; turn++) {
      ...
      if (dump) {
        fprintf(dump, "### Turn: %d; Side: %d; Samurai: %d; Actions:", turn,
                activeSide, activeSamurai);
      }
      ...
      gameState.receiveActionCommands(fromAI, activeSide, activeSamurai,
                                      logOutput);
      if (dump) {
        fprintf(dump, "\n");
        ...
        for (int a = 0; a != 2; a++) {
          for (int w = 0; w != 3; w++) {
            ...
            fprintf(dump, "Samurai %d.%d@(%d,%d)", ss.side, ss.weapon,
