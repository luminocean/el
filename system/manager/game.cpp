...
void SamuraiState::init(Setting &setting, BattleField &field, int id) {
  side = id / 3;
  weapon = id % 3;
...
bool SamuraiState::move(GameState &state, int direction, string &comments) {
  ...
  if (p == 0) {
    comments += "Invalid move direction: " + to_string(direction) + " from " +
                position->coords.toString();
    return false;
  }
  for (int s = 0; s != 6; s++) {
    SamuraiState &ss = samuraiStates[s];
    if (&ss != this && ss.role->homeX == p->coords.x &&
        ss.role->homeY == p->coords.y) {
      comments +=
          "Moving to home of another samurai: " + position->coords.toString();
      return false;
    }
  }
  if (hidden) {
    if (p->state < 0 || p->state / 3 != role->id / 3) {
      comments += "Hidden move to non-territory from: " +
                  position->coords.toString() + " to " + p->coords.toString();
      return false;
    }
  } else {
    if (p->apparent != 0) {
  ...
  if (dump) {
    fprintf(dump, "Samurai %d.%d moves from %s to %s\n", side, weapon,
            position->coords.toString().c_str(), p->coords.toString().c_str());
  }
  position->leave(this);
  position = p;
  position->arrive(this);
...
void GameState::sendGameInfo(FILE *out, int id) {
  fprintf(out, "# <# turns> <side> <weapon> <width> <height> <cure period>\n"
               "%d %d %d %d %d %d\n",
          setting.turns, id / 3, id % 3, battleField.width, battleField.height,
          setting.cureTurns);
...
void GameState::sendTurnInfo(FILE *out, int side, int weapon) {
  ...
  if (ss.alive) {
    fprintf(out, "# Turn information for samurai of side %d with weapon %d\n",
            side, weapon);
    fprintf(out, "# <turn>\n%d\n", turn);
    ...
    memset(visible, width * height * sizeof(bool), 0);
    for (int w = 0; w != 3; w++) {
      int who = 3 * side + w;
    ...
    for (int y = 0; y != height; y++) {
      for (int x = 0; x != width; x++) {
        if (visible[y * width + x]) {
          ...
          int sectState =
              (sect->state == -1 ? 8 : (side == 0 ? sect->state
                                                  : (sect->state >= 3
                                                         ? sect->state - 3
                                                         : sect->state + 3)));
...
void GameState::receiveActionCommands(SamuraiScanner &is, const int side,
                                      const int weapon, FILE *log) {
  ...
  if (ss.alive) {
    ...
    chrono::system_clock::time_point till =
        chrono::system_clock::now() + chrono::milliseconds(setting.timeAllowed);
    while (!ss.done) {
    ...
    fprintf(log, "# Actions\n");
    {
      ...
      while (!ss.actions.empty()) {
        ...
        if (action < 0 || action > 10) {
          ...
          ss.actions.pop();
          if (action <= 4) {
            if (!ss.occupy(*this, action - 1, turnComments))
              goto FINISH;
          } else if (action <= 8) {
            if (!ss.move(*this, action - 5, turnComments))
              goto FINISH;
          } else if (action == 9) {
            if (!ss.hide(turnComments))
              goto FINISH;
          } else if (action == 10) {
            if (!ss.appear(turnComments))
              goto FINISH;
          }
